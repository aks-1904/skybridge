...
import { useTrips, useShipments } from "../hooks/useSkybridge";

const CarrierDashboard = () => {
  const { trips, loading: tripsLoading } = useTrips();
  const { shipments, loading: shipmentsLoading } = useShipments(); // Fetches relevant shipments
  const navigate = useNavigate();

  // Filter shipments that are actively being carried (matched/in_transit)
  const carriedShipments = shipments.filter(
    (s) => s.status === "matched" || s.status === "in_transit"
  );
